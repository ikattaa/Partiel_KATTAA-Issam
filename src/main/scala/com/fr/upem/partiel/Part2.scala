...
import com.fr.upem.partiel.Part2.BankAccount
...
object Part2 {
  ...
  case class BankAccount(id :Int,c : String)
  case object User
    case class Transaction(id:Int, amount : Double, date : String)
    trait Categories
      object Categories {
  ...
  def addTransaction(a:Double,d:String) : Nothing = ???


  // 2.3 Use the api that you just created.
  // - Create an empty account
      val bankAccount = new BankAccount(_,_)
  ...
  // 4,,24,1546612190770
    def setFile(file : File) : Unit = {
      ...
      val csvWriter = new CSVWriter(outputFile) // CSVWriter ne veut pas marcher du coup je me suis bloqué parce que normalement ca marche.
  }
  ...
  // 1,invalid type,invalid amount,invalid date
    def getFile(f : File) : Option[BankAccount] = {
      val input = Source.fromFile(f)
      val lines = input.getLines().toList
      if(!lines.isEmpty) {
        val t = lines(0).split(",")
        if(t.length < 1) None else Some(BankAccount(t(0).toInt,t(1)))
      }
      else None
  }
